...
files = [
    f'../pickles/mcsolve_three_outcome_state{0}.pkl',
    f'../pickles/mcsolve_three_outcome_state{1}.pkl',
    f'../pickles/mcsolve_three_outcome_state{2}.pkl',
    f'../pickles/mcsolve_three_outcome_state{3}.pkl',
    f'../pickles/mcsolve_three_outcome_state{4}.pkl',
    f'../pickles/mcsolve_three_outcome_state{5}.pkl',
    f'../pickles/mcsolve_three_outcome_state{6}.pkl',
    f'../pickles/mcsolve_three_outcome_state{7}.pkl',
]
for i, file in enumerate(files):
    ...
    with open(f'../pickles/mcsolve_three_outcome_state{i}_summed.pkl', 'wb') as file:
...
for i, system in zip([0,1, 2,3,4,5,6,7],[system_leak0, system_leak3, system_computational,system_computational,system_computational,system_computational,system_computational,system_computational]):
    with open(f'../pickles/mcsolve_three_outcome_state{i}_summed.pkl', 'rb') as file:
        result = pickle.load(file)
    states = []
    for state in tqdm(result.states,desc=f'padding states from result {i}'):
        states.append(system.pad_back_function(state))
    result.states = states
    with open(f'../pickles/mcsolve_three_outcome_state{i}_summed_padd_back.pkl', 'wb') as file:
        pickle.dump(result, file)
...
for i, system in zip([0,1,2,3,4,5,6,7],[system_leak0, system_leak3, system_computational,system_computational,system_computational,system_computational,system_computational,system_computational]):
    ...
    result.states = states
    with open(f'../pickles/mcsolve_three_outcome_state{i}_summed_two_level.pkl', 'wb') as file:
        pickle.dump(result, file)