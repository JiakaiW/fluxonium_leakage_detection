...
def main():
    ...
    json_files = [f for f in os.listdir('.') if f.startswith('nevergrad_optimized_ef_X_') and f.endswith('.json')]
    
    # Read each file and store in the data structure
    for filename in json_files:
        # Extract detuning and t_duration from filename
        parts = filename.replace('nevergrad_optimized_ef_X_', '').replace('.json', '').split('_')
        t_tot = float(parts[0])
        ...
        if t_tot not in all_results:
            all_results[t_tot] = result
